...
import {getAllDataAPI, editProfileDataAPI, createCardAPI, editAvatarAPI} from './api.js';
...
const handleProfileFormSubmit = async (event) => {
    ...
    try {
        ...
        const remoteProfileData = await editProfileDataAPI(
            {'name': profileNameField.value, 'about': profileDescriptionField.value}
        );
...
const placeCards = (cardsDataArray) => {
    cardsDataArray.forEach((card) => placeList.append(partialCreateCard(card, profileID)));
};

// Загружает профиль на страницу
const placeProfileData = (ProfileDataObj, props = ['name', 'about', 'avatar']) => {
    props.includes('name') && (profileTitle.textContent = ProfileDataObj.name);
    props.includes('about') && (profileDescription.textContent = ProfileDataObj.about);
    props.includes('avatar') && (profileAvatar.style.backgroundImage = `url('${ProfileDataObj.avatar}')`);
};
